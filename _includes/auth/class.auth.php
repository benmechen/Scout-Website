  ...
  * @param array $params      -- additional params
	...
	*/
	...
	private function addRequest($uid, $email, $type, &$sendmail)
	{
		...
		if($sendmail === true)
        {
			...
			//Send activation email to one in config, as using usernames instead of emails, otherwise a system error is thrown
			$mail->addAddress($this->config->activate_email_to);
			$mail->isHTML(true);
  ...
  //       }
	...
	*/
	...
	/**
     * Get user's pemission value with given id
     * @param int $userid
     * @return int
     */    
	public function getPermission($hash){
		$query = $this->dbh->prepare("SELECT uid FROM {$this->config->table_sessions} WHERE hash = ?");
		$query->execute(array($hash));
		...
		$query = $this->dbh->prepare("SELECT permission FROM {$this->config->table_users} WHERE id = ?");
		$query->execute(array($uid));
	...
	/**
     * Change user's pemission value with given id
     * @param int $userid
     * @return int
     */
	public function changePermission($uid, $newpermission){
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET permission = ? WHERE id = ?");
		$query->execute(array($newpermission, $uid));

		$return['error'] = false;
		$return['message'] = $this->lang["permisson_changed"];
		return $return;
