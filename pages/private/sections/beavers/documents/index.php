<?php
	...
	if ($logged = true) {
		...
		if(isset($_POST['documents-delete-submit'])){
			...
			$query = $documentsDBH->prepare("DELETE FROM beavers WHERE title = ?");
		...
		if (isset($_POST['rename-submit'])) {
			...
			$query = $documentsDBH->prepare("UPDATE beavers SET title = ?  WHERE title = ?");
...
<html>
	<head>
		<title>1st Lytchett Matravers Scout Group | Beavers - Documents</title>
				...
				<div class="beavers-page-sidebar main-side">
					...
					<div class="anim_element anim_element--fadeIn">
						...
						?> <p><b>Recently Uploaded:</b> 
							<ul><?
								$query = $documentsDBH->prepare("SELECT title, date FROM beavers ORDER BY id DESC");
...
<script type="text/javascript">
    ...
    .bind('fileuploadcompleted', function (e, data) {console.log("Completed");})
    .bind('fileuploadfail', function (e, data) {console.log("Failed");})
