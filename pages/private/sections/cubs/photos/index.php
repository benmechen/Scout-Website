<?php
	...
	if ($logged = true) {
		...
		if (!empty($_POST['photos-folder-submit'])) {
		    ...
		    if (isset($error) && in_array(1, $error)) {
				...
				if ($photosCreate['success'] == true) {
					...
					$groups->mailUsers("cubs", "photos", htmlspecialchars($_POST['photos-folder-title']), htmlspecialchars($_POST['photos-folder-description']));
				}
		...
		if (isset($_POST['rename-submit'])) {
			...
			$photosFolders->update($_POST['rename-id'], $_POST['rename-newname'],$_PUBLIC_DIR . "pages/private/sections/cubs/photos/", $_POST['rename-oldname']);
...
<html>
	<head>
		<title>1st Lytchett Matravers Scout Group | Cubs</title>
...
<?
		$seen = $notifications->seen($photosDBH, "cubs");
	}
