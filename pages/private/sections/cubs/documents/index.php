<?php
	...
	if ($logged = true) {
		...
		// $photosFolders = new Dir('cubs', $photosDBH);
		...
		if(isset($_POST['documents-delete-submit'])){
			...
			$query = $documentsDBH->prepare("DELETE FROM cubs WHERE title = ?");
		...
		if (isset($_POST['rename-submit'])) {
			...
			$query = $documentsDBH->prepare("UPDATE cubs SET title = ?  WHERE title = ?");
...
<html>
	<head>
		<title>1st Lytchett Matravers Scout Group | Cubs - Documents</title>
				...
				<div class="cubs-page-sidebar main-side">
					...
					<div class="anim_element anim_element--fadeIn">
						...
						?> <p><b>Recently Uploaded:</b> 
							<ul><?
								$query = $documentsDBH->prepare("SELECT title, date FROM cubs ORDER BY id DESC");
