	...
	if ($logged = true) {
		...
		if (!empty($_POST['news-submit'])) {
		    ...
		    if (isset($error) && in_array(1, $error)) {
				...
				if ($newsPost == true) {
					$groups->mailUsers("cubs", "post", htmlspecialchars($_POST['news-title']), htmlspecialchars($_POST['news-comment']));
...
<?
		$seen = $notifications->seen($newsDBH, "cubs");
	}
