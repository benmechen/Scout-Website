	...
	if ($logged = true) {
		...
		if (!empty($_POST['news-submit'])) {
			...
			$newsPost = $news->push(htmlspecialchars($_POST['news-title']), htmlspecialchars($_POST['news-comment']), date('l jS \of F Y'));
				if ($newsPost == true) {
					$groups->mailUsers("scouts", "post", htmlspecialchars($_POST['news-title']), htmlspecialchars($_POST['news-comment']));
...
<?
		$seen = $notifications->seen($newsDBH, "scouts");
	}
