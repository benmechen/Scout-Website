<?php
	...
	if ($logged = true) {
		...
		// Setup news
		$news = new News("explorers", $newsDBH);
		...
		if (!empty($_POST['news-submit'])) {
		    ...
		    if (isset($error) && in_array(1, $error)) {
				...
				if ($newsPost == true) {
					$groups->mailUsers("explorers", "post", htmlspecialchars($_POST['news-title']), htmlspecialchars($_POST['news-comment']));
...
<?
		$seen = $notifications->seen($newsDBH, "explorers");
	}
