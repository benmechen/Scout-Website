<?php
	...
	if ($logged = true) {
		...
		if(isset($_POST['documents-delete-submit'])){
			...
			$query = $documentsDBH->prepare("DELETE FROM explorers WHERE title = ?");
		...
		if (isset($_POST['rename-submit'])) {
			...
			$query = $documentsDBH->prepare("UPDATE explorers SET title = ?  WHERE title = ?");
...
<html>
	<head>
		<title>1st Lytchett Matravers Scout Group | Explorers - Documents</title>
				...
				<div class="explorers-page-sidebar main-side">
					...
					<div class="anim_element anim_element--fadeIn">
						...
						?> <p><b>Recently Uploaded:</b> 
							<ul><?
								$query = $documentsDBH->prepare("SELECT title, date FROM explorers ORDER BY id DESC");
