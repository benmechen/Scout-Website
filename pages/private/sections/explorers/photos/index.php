<?php
	...
	if ($logged = true) {
		...
		$photosFolders = new Dir('explorers', $photosDBH);
		...
		if (!empty($_POST['photos-folder-submit'])) {
		    ...
		    if (isset($error) && in_array(1, $error)) {
				...
				if ($photosCreate['success'] == true) {
					...
					$groups->mailUsers("explorers", "photos", htmlspecialchars($_POST['photos-folder-title']), htmlspecialchars($_POST['photos-folder-description']));
				}
		...
		if (isset($_POST['rename-submit'])) {
			...
			$photosFolders->update($_POST['rename-id'], $_POST['rename-newname'],$_PUBLIC_DIR . "pages/private/sections/explorers/photos/", $_POST['rename-oldname']);
...
<html>
	<head>
		<title>1st Lytchett Matravers Scout Group | Explorers</title>
					...
					<div class="grid">
						<?
							...
							foreach ($photosData as $data) {
								...
								// var_dump($photosThumbnail);
								// echo $photosThumbnail;
								?>
								<?= $message ?>
								<div class="grid-item folder-grid-item">
									<a href="<?= $data['title'] ?>">
										<div class="photos-tile" style="background-image: url('<?= $photosThumbnail ?>'); background-color: rgba(153,153,153,9);background-size: cover; background-repeat:no-repeat; background-position: center;">
											<span class="explorers-photos-name">
...
<?
		$seen = $notifications->seen($photosDBH, "explorers");
	}
